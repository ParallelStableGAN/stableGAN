...
parser.add_argument('--dist-url',
                    default='file:///lustre/cmsc714-1o01/stableGAN', type=str,
                    help='url used to set up distributed training')
parser.add_argument('--dist-backend', default='nccl', type=str,
                    help='distributed backend')
parser.add_argument('--dist-group', default='', type=str,
...
def main():
    ...
    if opt.distributed:
        ...
        dist.init_process_group(
            backend=opt.dist_backend, init_method=opt.dist_url,
    ...
    if torch.cuda.is_available() and not opt.cuda:
        print("WARNING: You have a CUDA device, probably run with --cuda")
    ...
    # setup optimizer
    optimizerD = AdamPre(netD.parameters(), lr=opt.lr/opt.DLRatio,
                         betas=(opt.beta1, 0.999), name='optD')
    optimizerG = AdamPre(netG.parameters(), lr=opt.lr/opt.GLRatio,
    ...
    for i in range(opt.niter):
        ...
        real_cpu = generate_data_SingleBatch(
            ...
            sigma=opt.sigma, batchSize=opt.batchSize)
        ...
        if i % opt.viz_every == 0 or i == opt.niter - 1:
            ...
            # save checkpoints
            torch.save(netG.state_dict(), '{0}/netG_epoch_{1}.pth'.format(
                opt.outf, i))
            torch.save(netD.state_dict(), '{0}/netD_epoch_{1}.pth'.format(
                opt.outf, i))
            ...
            if opt.markerSize:
                plt.scatter(tmp_cpu[:, 0], tmp_cpu[:, 1], c='g',
                            edgecolor='none', s=opt.markerSize)
            else:
                plt.scatter(tmp_cpu[:, 0], tmp_cpu[:, 1], c='g',
                            edgecolor='none')
    ...
    #                       cmap='Greens',
    #                       n_levels=nLevels, clip=[[-xmax, xmax]]*2)
